    ...
    <div class="min-h-screen bg-borovnica-light py-8 sm:py-10">
        <div class="mx-auto max-w-3xl px-4 sm:px-6">
            <div class="mb-8 flex flex-col items-center">
                <h1 class="text-center text-3xl font-bold italic uppercase tracking-widest text-white">Izmena proizvoda</h1>
            ...
            <div class="rounded-sm border border-borovnica-dark/20 bg-borovnica-table p-5 font-semibold text-borovnica-dark shadow-2xl sm:p-8">
                ...
                <form action="{{ route('proizvod.update', $proizvod) }}" method="POST">
                    @csrf
                    @method('PUT')
                    @include('proizvod.partials.form', ['submitLabel' => 'Sačuvaj izmene'])
                </form>
            </div>
        </div>
    </div>
